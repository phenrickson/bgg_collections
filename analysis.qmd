...
To view games predicted by the model, go to @sec-predictions.
...
preds = get(ls(pattern = user_preds))
...
preds |>
        ...
        mutate(roc_curve = map(data, safely(safe_roc_curve))) |>
...
preds |>
        ...
        predictions_datatable(remove_description = T, remove_image = T, pagelength = 15)
```
...
preds |>
        filter(type %in% c('resamples', 'valid', 'test')) |>
        filter(yearpublished <= max(yearpublished)-2) |>
        top_n_preds(
                games = games,
                top_n = 15
        ) |>
